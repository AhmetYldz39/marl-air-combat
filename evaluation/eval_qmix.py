...
eval_qmix.py
============
QMIX checkpoint'ini heuristic rakibe karşı değerlendirir.

Kullanım:
    python -X utf8 evaluation/eval_qmix.py --checkpoint checkpoints/qmix_ep2000.pt
    python -X utf8 evaluation/eval_qmix.py --checkpoint checkpoints/qmix_ep2000.pt --episodes 500 --seed 42
"""
...
from models.qmix_net import AgentQNetwork, ActionMapper
...
def run_eval(config: dict, checkpoint_path: str, n_episodes: int,
    ...
    print(f"[Eval] QMIX checkpoint : {checkpoint_path}")
    ...
    kills_per_ep   = []
    lengths        = []
    double_kills   = 0      # 2nd_kill_rate: her iki red de ölmüş mü
    ...
    for ep in range(n_episodes):
        ...
        while not done["__all__"]:
            # QMIX: argmax(Q) → ActionMapper → 5D continuous
            with torch.no_grad():
                blue_actions = {}
                for aid in blue_ids:
                    ...
                    blue_actions[aid] = mapper(idx)

            # Heuristic: tüm ajanlar için al, yalnızca red_ids kullan
...
def print_summary(r: dict):
    print("\n" + "=" * 55)
    print(f"  QMIX EVAL — {r['n_episodes']} episode | seed={r['seed']}")
    print("=" * 55)
...
def main():
    ...
    p.add_argument("--checkpoint", default="checkpoints/qmix_ep2000.pt")
    p.add_argument("--config",     default="configs/config.yaml")
    p.add_argument("--episodes",   type=int, default=500)
    p.add_argument("--seed",       type=int, default=42)
    p.add_argument("--device",     default="auto")
    p.add_argument("--output",     default="logs/eval_qmix_results.json")
    args = p.parse_args()
